...
def main():
    ...
    path = 'experiments/cifar10/RemixMatch/result5_50_50/model_400.pth.tar'
    checkpoint = torch.load(path)
    model.load_state_dict(checkpoint['state_dict'])
    ema_model.load_state_dict(checkpoint['ema_state_dict'])
    for param in model.parameters():
        param.requires_grad = False
    num_fc_ftr = model.output.in_features 
    model.output = nn.Linear(num_fc_ftr, 10) 
    model = model.cuda()
    ...
    optimizer = optim.Adam(model.output.parameters(), lr=args.lr, weight_decay=args.wd)
...
def train(labeled_trainloader, unlabeled_trainloader, model, optimizer, ema_optimizer, criterion, epoch, use_cuda,
    ...
    for batch_idx in range(args.val_iteration):
        ...
        with torch.no_grad():
            ...
            weight_u = class_weight_u.repeat(p.size()[0], 1)
            p = torch.mul(p, weight_u)
