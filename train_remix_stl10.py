...
def main():
    ...
    datasets = dataset.get_stl10('/root/data', N_SAMPLES_PER_CLASS, IMB_TEST_PER_CLASS,
                                                        method='remix')
    ...
    # emp_distb_u = torch.ones(num_class) / num_class
    # pseudo_orig = torch.ones(len(train_unlabeled_set.labels), num_class) / num_class
    # pseudo_refine = torch.ones(len(train_unlabeled_set.labels), num_class) / num_class
